...
size_t count_paths(const struct graph *g, size_t from, size_t to, char visited[], int used_twice)
{
    if (from == to) {
        return 1;
    } else {
        ...
        for (i = 0; i < g->adj_lens[from]; i++) {
            ...
            int was_visited = visited[neighbor];
            int is_small = g->is_small[neighbor];
            if (neighbor != g->start_i && !(is_small && was_visited && used_twice)) {
                if (is_small && !was_visited)
                    visited[neighbor] = 1;
                total += count_paths(g, neighbor, to, visited, used_twice || was_visited);
                if (is_small && !was_visited)
                    visited[neighbor] = 0;
            }
        }
...
int main(void)
{
    ...
    g.start_i = start_i;
    visited[start_i] = 1;
    printf("%lu\n", count_paths(&g, start_i, end_i, visited, 0));
