...
int main(void)
{
    ...
    char board_already_won[MAX_BINGOS] = {}; // Another set
    size_t i, j;
    long last_win_board = -1, last_win_number = -1;

    for (i = 0; i < n_called; i++) {
        called[called_numbers[i]] = 1;
        for (j = 0; j < n_boards; j++) {
            if (!board_already_won[j] && board_won(boards[j], called)) {
                last_win_board = j;
                last_win_number = i;
                board_already_won[j] = 1;
            }
        }
    }

    if (last_win_board >= 0) {
        for (long i = n_called - 1; i > last_win_number; i--)
            called[called_numbers[i]] = 0;
        printf("%ld\n", board_score(boards[last_win_board], called, called_numbers[last_win_number]));
    } else {
