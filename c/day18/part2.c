...
#define N 1024 // 1 Kebi
#define MAX_SNAILS 1024
...
void sncopy(struct snailnum *dst, const struct snailnum *src)
{
    dst->len = src->len;
    memcpy(dst->elems, src->elems, src->len * sizeof(src->elems[0]));
}

uint64_t add_mag(const struct snailnum *sn1, const struct snailnum *sn2)
{
    static struct snailnum num;
    sncopy(&num, sn1);
    add_snailnum(&num, sn2);
    reduce(&num);
...
int main(void)
{
    static struct snailnum nums[MAX_SNAILS];
    size_t i, j, len = 0;
    uint64_t max_mag = 0;

    while (read_snailnum(&nums[len])) len++;
   
    for (i = 0; i < len; i++) {
        for (j = i + 1; j < len; j++) {
            max_mag = max(max_mag, add_mag(&nums[i], &nums[j]));
            max_mag = max(max_mag, add_mag(&nums[j], &nums[i]));
        }
    }
