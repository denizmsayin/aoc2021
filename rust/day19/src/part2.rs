...
fn match_scanners(s0: &[I3], s1: &[I3], rots: &[Rotation]) -> Option<TranslRot> {
    let mut max_match = 0;
...
fn generate_direct_transforms(tfs: &HashMap<(usize, usize), TranslRot>) -> HashMap<usize, TranslRot> {
    ...
    let mut direct_tfs: HashMap<usize, TranslRot> = HashMap::new();
    ...
    while let Some(cur) = q.pop_front() {
        ...
        for &ni in neighbors.iter() {
            if !direct_tfs.contains_key(&ni) {
                let tf = &tfs[&(ni, cur)];
                let direct_tf = direct_tfs[&cur].comp(tf); 
    ...
    for i in 0..scanners.len() {
        for j in i+1..scanners.len() {
            if let Some(etf) = match_scanners(&scanners[i], &scanners[j], &face_tfs) {
                scanner_tfs.insert((i, j), etf.inv());
                scanner_tfs.insert((j, i), etf);
            }
        }
    }
